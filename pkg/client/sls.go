...
func NewAliCloudSlsClient(source models.AlertDataSource) AliCloudSlsClientApi {
	...
	if err != nil {
		global.Logger.Sugar().Errorf("创建 SLS 客户端失败 -> %s", err.Error())
...
func GetSLSBodyData(res *sls20201230.GetLogsResponse) SlsBody {
	...
	for _, body := range res.Body {
		...
		if err != nil {
			global.Logger.Sugar().Errorf("解析 SLS Metric Label 失败, %s", err.Error())
		}
