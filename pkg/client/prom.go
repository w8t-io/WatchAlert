...
func NewPromClient(source models.AlertDataSource) API {
	client, err := api.NewClient(api.Config{
		Address: source.HTTP.URL,
	})
	if err != nil {
		global.Logger.Sugar().Errorf("Prometheus 初始化客户端失败: %s", err)
	}
...
func (a API) Query(promQL string) ([]Vector, Warnings, error) {
	...
	if err != nil {
		global.Logger.Sugar().Errorf("Prometheus 执行query失败: %s", err)
