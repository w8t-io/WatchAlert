...
func NewJaegerClient(datasourceId string) JaegerClient {
	var data models.AlertDataSource
	err := globals.DBCli.Model(&models.AlertDataSource{}).Where("id = ?", datasourceId).First(&data).Error
	if err != nil {
		globals.Logger.Sugar().Errorf(err.Error())
		return JaegerClient{}
	}

	_, err = http.Get(data.HTTP.URL)
	if err != nil {
		globals.Logger.Sugar().Errorf(err.Error())
		return JaegerClient{}
	}

	return JaegerClient{BaseURL: data.HTTP.URL}
}
...
func (j JaegerData) GetAnnotations(rule models.AlertRule) string {
	return fmt.Sprintf("\n服务: %s 链路中存在异常状态码接口\nJaeger URL: %s/trace/%s\n\n详情查看 Jaeger Trace ⬆️", rule.JaegerConfig.Service, globals.Config.Jaeger.URL, j.TraceID)
}
